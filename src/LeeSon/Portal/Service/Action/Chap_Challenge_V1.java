...
public class Chap_Challenge_V1 {
	...
	public byte[] Action(String Bas_IP, int bas_PORT, int timeout_Sec,
		...
		System.out.println("REQ Challenge"
				+ WR.Getbyte2HexString(Req_Challenge));
		...
		try {
			...
			// 接收服务器的数据包
			byte[] ACK_Challenge_Data = new byte[34];
			DatagramPacket receivePacket = new DatagramPacket(
					ACK_Challenge_Data, 34);
			// 设置请求超时3秒
			dataSocket.setSoTimeout(timeout_Sec * 1000);
			dataSocket.receive(receivePacket);

			System.out.println("ACK Challenge"
					+ WR.Getbyte2HexString(ACK_Challenge_Data));
